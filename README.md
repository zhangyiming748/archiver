...
- 🎵 **Opus 转换**：将音频文件批量转换为高压缩率的 Opus 格式
...
# 指定目录
archiver opus --dir /path/to/audio

# 简写形式
archiver opus -d /path/to/audio
```
...
- 默认在当前目录（`.`）下搜索音频文件并进行转换
- 使用 libopus 编码器（160k 码率），自动将多声道降级为立体声并剔除元数据，已是 `.opus` 的文件会跳过
- 转换成功后会删除源文件，建议先备份重要音频
...
- **`opus`**：将音频文件转换为 Opus 格式（不加速、不调电平）
...
| `archiver opus` | 音频转 Opus 格式 | `-d` (目录) |
